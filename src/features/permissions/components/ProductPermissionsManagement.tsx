...
const ProductPermissionsManagement = () => {
  ...
  const [currentPermission, setCurrentPermission] =
    useState<ProductPermission | null>(null);
  ...
  const handleSubmit = async (data: PermissionFormValues) => {
    try {
      if (currentPermission) {
        ...
        // Create new permission
        const { error } = await supabase.from("product_permissions").insert({
          name: data.name,
          description: data.description,
        });
    ...
    try {
      ...
      if (rolePermissions && rolePermissions.length > 0) {
        toast({
          ...
          description:
            "This permission is assigned to one or more roles. Remove these assignments first.",
        });
        return;
      }
          ...
          <TableBody>
            {permissions.length > 0 ? (
              ...
              <TableRow>
                ...
                >
                  No permissions found. Create your first permission by clicking
                  'Add New Permission'.
                </TableCell>
              </TableRow>
            )}
