            ...
            {filteredAssignments.length > 0 ? (
              filteredAssignments.map((assignment) => (
                <TableRow key={assignment.id}>
                  <TableCell>
                    <div>
                      <div className="font-medium">{assignment.user?.email}</div>
                      {assignment.user?.user_metadata.name && (
                        <div className="text-sm text-gray-500">{assignment.user.user_metadata.name}</div>
                      )}
                    </div>
                  </TableCell>
                  <TableCell>{assignment.product?.name || 'Unknown Product'}</TableCell>
                  <TableCell>
                    <Badge variant="outline">{assignment.role?.name || 'Unknown Role'}</Badge>
                  </TableCell>
              ...
              <TableRow>
                <TableCell colSpan={4} className="text-center py-8 text-gray-500">
                  {searchTerm ? 'No matching assignments found' : 'No role assignments found. Assign your first role by clicking "Assign Role".'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
