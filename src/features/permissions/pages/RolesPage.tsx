...
const permissionCategories = [
  {
    name: "Users & Access",
    items: ["user_management", "user_view", "role_management"],
  },
  ...
  { name: "Billing", items: ["billing_management"] },
  {
    ...
    items: ["customer_management", "ticket_management"],
  },
];
...
const RolesPage = () => {
  ...
  const countSelectedInCategory = (categoryItems: string[]) => {
    return categoryItems.filter((item) => selectedPermissions.includes(item))
      .length;
  };
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <Dialog
            ...
            onOpenChange={(open) => !open && setIsEditRoleDialogOpen(false)}
          >
            <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-hidden flex flex-col">
              <DialogHeader>
                <DialogTitle>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {allPermissions.map((permission) => {
                          const hasPermission = viewRole.permissions.includes(
                            permission.id,
                          );
                          const category = permissionCategories.find((cat) =>
                            cat.items.includes(permission.id),
                          );
