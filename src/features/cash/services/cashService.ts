...
import { categories as mockCategories, products as mockProducts } from "@/components/cash/mockData";
  ...
  try {
    ...
    if (data && data.length > 0) {
      return data.map((item) => ({
        id: item.id,
        name: item.name,
        icon: mockCategories.find(c => c.name === item.name)?.icon || mockCategories[0].icon
      }));
    }
...
export const getProducts = async (categoryName?: string | null): Promise<Product[]> => {
  try {
