...
const Logo: React.FC<LogoProps> = ({ className }) => {
  ...
  return (
    <div
      className={cn(
        "relative h-16 w-16 transform rotate-45",
        shadowClass,
        className,
      )}
    >
      ...
      <div className="absolute inset-0 grid grid-cols-2 grid-rows-2 gap-0">
        ...
        <div className="bg-brandRed rounded-sm shadow-md relative">
          ...
          <div
            className={`absolute bottom-0 left-0 w-full h-[2px] ${borderColor}`}
          />

          {/* Vertical border (right of red) */}
          <div
            className={`absolute top-0 right-0 h-full w-[2px] ${borderColor}`}
          />
        </div>
